...
const PlayTrailer = ({ videos, className = 'action-button' }) => {
        ...
        if (Array.isArray(videos) && videos?.length > 0) {
            ...
            if (trailers?.length > 0) {
                ...
                const sortedTrailers = trailers.sort((trailer1, trailer2) => {
                    return Date.parse(trailer1?.published_at) - Date.parse(trailer2?.published_at)
                });
            ...
            {(isLoading || videoId) && (
                ...
                <Link className={`${className}`}
                    ...
                    {isLoading
                        ...
                        : <span>
                            <i style={{ fontSize: '1.5rem' }} className="fab fa-youtube fa-lg"></i>
                        </span>
                    }
                </Link>
            )}
