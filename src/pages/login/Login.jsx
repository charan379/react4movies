...
const Login = () => {
  ...
  const forwardToRequestedRoute = async (requestedRoute, cancelToken) => {
    ...
    try {
      ...
      const {
        data: { userName, email, status, role, createdAt, updatedAt },
      } = await movieBunkersAPI.get(`/auth/who-am-i`, {
      ...
      toast.update(toastId, {
        render: "Forwarding to requested page...!",
        type: "info",
        isLoading: false,
        autoClose: 3000,
        delay: 1500,
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const {
        data: { message, token },
      } = await movieBunkersAPI.post(`/auth/token-auth`, {
        userName,
        password,
      });
      ...
      const message =
        errors?.response?.data?.error?.reason ??
        errors?.message ??
        "Something went wrong";
